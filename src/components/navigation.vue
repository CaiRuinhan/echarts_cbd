  ...
  <div class="box">
    ...
    >
      ...
      >
        ...
        <MenuItem name="17" v-if="echarts_history">
          <Icon type="document-text"></Icon>历史总览ES
        </MenuItem>
...
export default {
  data() {
    return {
      ...
      echarts_history: false, //历史总览echarts
    };
  },
  created() {
    // console.log('我是导航')
    ...
    this.menuSet.map((res) => {
      ...
      if (res == "echarts_history") {
        this.echarts_history = true;
      }
    });
    if (this.$route.path == "/holistic_analysis") {
      this.activemenu = "1";
    }
    if (this.$route.path == "/new_holistic_analysis") {
      this.activemenu = "2";
    }
    if (this.$route.path == "/newyard_analysis") {
      this.activemenu = "15";
    }
    if (
    ...
    if (this.$route.path == "/system_Journal") {
      this.activemenu = "9";
    }
    if (this.$route.path == "/system_setup") {
      this.activemenu = "10";
    }
    if (this.$route.path == "/menu_Jurisdiction") {
      this.activemenu = "11";
    }
    if (this.$route.path == "/equipment_Journal") {
      this.activemenu = "12";
    }
    if (this.$route.path == "/operation_record") {
      this.activemenu = "16";
    }
    if (
    ...
    if (this.$route.path == "/three_city") {
      this.activemenu = "three";
    }
    if (this.$route.path == "/echarts_history") {
      this.activemenu = "17";
    }
  },
  methods: {
